...
import { TranslationManager } from '../src/components/TranslationManager';

export default function AdminPage() {
  const router = useRouter();
  const { asPath } = router; // For canonical URL if ever needed, though not for noindex
                      ...
                      {filteredProducts.map((product) => (
                        <tr key={product.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                          ...
                          <td className="px-6 py-4"> {/* Removed whitespace-nowrap */}
                            <div className="max-w-xs"> {/* Added max-width to the container */}
                              <div className="text-sm font-medium text-gray-900 dark:text-white break-words"> {/* Added break-words */}
                                {product.name}
                              </div>
                              <div className="text-sm text-gray-500 dark:text-gray-400 truncate max-w-xs"> {/* Description can still truncate */}
                                {product.description}
                              </div>
                            </div>
                          </td>
