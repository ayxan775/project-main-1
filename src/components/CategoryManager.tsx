...
export function CategoryManager({ token, onCategoryChange }: CategoryManagerProps) {
  ...
  const { locale } = router;
  const t = locale === 'az' ? az.categoryManager : locale === 'ru' ? ru.categoryManager : en.categoryManager; // Select translations
  ...
  const handleSaveCategory = async (e: React.FormEvent) => {
    ...
    if (!formData.name.trim()) {
      setError(t.errorNameRequired); // Use translation
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || t.errorSave); // Use translation
      }
      ...
      setError(error instanceof Error ? error.message : t.errorSave); // Use translation
    }
  };

  // Delete category
  const handleDeleteCategory = async (id: number) => {
    if (!confirm(t.confirmDelete)) { // Use translation
      return;
    }
    ...
    try {
      ...
      if (errorData.canReassign && errorData.productsCount > 0) {
        ...
        if (otherCategories.length === 0) {
          alert(t.alertCannotDelete); // Use translation
          return;
        }

        // Ask user which category to reassign products to
        const confirmReassignMsg = t.confirmReassign
          .replace('{count}', errorData.productsCount);
        const reassign = confirm(confirmReassignMsg); // Use translation

        if (reassign) {
          // Create a dropdown for selecting the target category
          // Note: Using prompt is basic, a proper modal/select is better UI
          const promptMsg = t.promptReassign
            .replace('{count}', errorData.productsCount);
          const targetCategory = prompt(promptMsg, otherCategories[0].name); // Use translation
          ...
          if (targetCategory && otherCategories.some(cat => cat.name === targetCategory)) {
            ...
            if (reassignResponse.ok) {
              ...
              const successMsg = t.alertReassignSuccess
                .replace('{count}', result.reassigned)
                .replace('{categoryName}', targetCategory);
              alert(successMsg); // Use translation
              await fetchCategories();
              ...
              setError(reassignError.message || t.errorReassign); // Use translation
            }
          }
        }
        return;
      ...
      throw new Error(errorData.message || t.errorDelete); // Use translation
    } catch (error) {
    ...
    <div className="space-y-8">
      <div className="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6 border border-gray-200 dark:border-gray-700">
        <h2 className="text-lg font-medium text-gray-900 dark:text-white mb-4 pb-2 border-b border-gray-200 dark:border-gray-700">
          {editingCategory ? t.formTitleEdit : t.formTitleAdd} {/* Use translation */}
        </h2>
        ...
        <form onSubmit={handleSaveCategory} className="space-y-6">
          ...
          <div className="flex space-x-3">
            ...
            >
              ...
              {editingCategory ? t.buttonUpdate : t.buttonSave} {/* Use translation */}
            </button>
      ...
      <div className="bg-white dark:bg-gray-800 shadow-md rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700">
        <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-750">
          <h2 className="text-lg font-medium text-gray-900 dark:text-white">{t.tableTitle}</h2> {/* Use translation */}
        </div>

        {loading ? (
          <div className="p-6 text-center">
            <p className="text-gray-500 dark:text-gray-400">{t.loading}</p> {/* Use translation */}
          </div>
        ) : categories.length === 0 ? (
          <div className="p-6 text-center">
            <p className="text-gray-500 dark:text-gray-400">{t.noCategories}</p> {/* Use translation */}
          </div>
        ) : (
              ...
              <thead className="bg-gray-50 dark:bg-gray-750">
                <tr>
                  <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                    {t.headerId} {/* Use translation */}
                  </th>
                  <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                    {t.headerName} {/* Use translation */}
                  </th>
                  <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                    {t.headerDescription} {/* Use translation */}
                  </th>
